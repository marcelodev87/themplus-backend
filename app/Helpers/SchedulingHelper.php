...
{
    public static function allowCreateScheduling(string $enterpriseId, $period): void
    {
        $year = $period->year;
        $month = $period->month;
        ...
        if ($subscriptionName === 'free') {
            ...
            $countScheduling = DB::table('schedulings')
                ->where('enterprise_id', $enterpriseId)
                ->whereYear('date_movement', $year)
                ->whereMonth('date_movement', $month)
                ->count();

            if ($countScheduling >= 50) {
                throw ValidationException::withMessages([
                    'limit' => [
                        'Plano free permite no máximo 50 agendamentos por mês',
                    ],
                ]);
            }
        }
